...
async function seedArea(kecamatanId:number,list:Array<{nama:string;kode:string;rwCount:number}>){
  for(const kel of list){
    // upsert kelurahan
    const k = await prisma.kelurahan.upsert({
      where:{kecamatanId_nama:{kecamatanId,nama:kel.nama}},
      update:{kode:kel.kode},
      create:{kecamatanId,nama:kel.nama,kode:kel.kode}
    });

    // batch create RW
    const rwNomors = Array.from({length:kel.rwCount},(_,i)=>pad(i+1));
    await prisma.rW.createMany({
      data: rwNomors.map(nomor=>({kelurahanId:k.id,nomor})),
      skipDuplicates: true
    });

    // get all RW ids
    const rws = await prisma.rW.findMany({
      where:{kelurahanId:k.id},
      select:{id:true,nomor:true}
    });

    // batch create RT for all RW
    const rtData: Array<{rwId:number;nomor:string;jumlahKk:number;targetWarga:number}> = [];
    for(const rw of rws){
      const rwNum = parseInt(rw.nomor);
      const rtCount = getRtCount(rwNum);
      for(let rt=1;rt<=rtCount;rt++){
        const kk = 40+Math.floor(Math.random()*41);
        rtData.push({rwId:rw.id,nomor:pad(rt),jumlahKk:kk,targetWarga:kk*3});
      }
    }
    await prisma.rT.createMany({data:rtData,skipDuplicates:true});
    process.stdout.write(".");
  }
}

async function main(){
  console.log("Resuming Dapil 3 seed (batch mode)...");

  const prov = await prisma.provinsi.upsert({where:{nama:"DKI Jakarta"},update:{},create:{nama:"DKI Jakarta",kode:"DKI"}});
  const jakut = await prisma.kota.upsert({where:{provinsiId_nama:{provinsiId:prov.id,nama:"Jakarta Utara"}},update:{},create:{provinsiId:prov.id,nama:"Jakarta Utara",kode:"3172",tipe:"kota"}});
  const jakbar = await prisma.kota.upsert({where:{provinsiId_nama:{provinsiId:prov.id,nama:"Jakarta Barat"}},update:{},create:{provinsiId:prov.id,nama:"Jakarta Barat",kode:"3173",tipe:"kota"}});
  const kep = await prisma.kota.upsert({where:{provinsiId_nama:{provinsiId:prov.id,nama:"Kepulauan Seribu"}},update:{},create:{provinsiId:prov.id,nama:"Kepulauan Seribu",kode:"3101",tipe:"kabupaten"}});
