...
export async function tpsRoutes(app: FastifyInstance) {
  ...
  app.post('/list', { preHandler: [app.authenticate] }, async (req, reply) => {
    ...
    const tps = await prisma.tps.create({
      data: { kodeTps: body.kodeTps, electionId: +body.electionId, rtId: body.rtId ? +body.rtId : undefined, kelurahanId: body.kelurahanId ? +body.kelurahanId : undefined, kecamatanId: body.kecamatanId ? +body.kecamatanId : undefined, alamat: body.alamat, jumlahDpt: Number(body.jumlahDpt ?? 0) },
    });
  ...
  app.post('/results', { preHandler: [app.authenticate] }, async (req, reply) => {
    ...
    const result = await prisma.tpsResult.upsert({
      ...
      create: { tpsId: +body.tpsId, electionId: +body.electionId, jumlahPengguna: Number(body.jumlahPengguna ?? 0), suaraSah: Number(body.suaraSah ?? 0), suaraTidakSah: Number(body.suaraTidakSah ?? 0), hasilSuara: body.hasilSuara ?? {}, buktiFotoUrl: body.buktiFotoUrl, status: 'submitted', inputBy: user.sub },
    });
